        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
            <div className="px-3 py-2 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              ...
              {eyeResult?.evidence?.gradCam?.image && <button onClick={() => setViewerState({isOpen: true, url: (eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam)?.startsWith('data:') || (eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam)?.startsWith('/') ? (eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam) || '' : `data:image/jpeg;base64,${(eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam) || ''}`, title: title + ' - Grad-CAM Attention'})} className="text-xs text-blue-600 hover:text-blue-800 font-medium" title="View full image">View</button>}
            </div>
            <div className="p-2 flex items-center justify-center bg-gray-900 h-32 flex-1">
              { (eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam) ? <img src={(eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam)?.startsWith('data:') || (eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam)?.startsWith('/') ? (eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam) || '' : `data:image/jpeg;base64,${(eyeResult?.evidence?.gradCam?.image || (eyeResult as any)?.gradCam) || ''}`} alt="Grad-CAM" className="max-h-full object-contain rounded" /> : <p className="text-xs text-gray-400">N/A</p>}
            </div>
          </div>
        </div>
      </div>
