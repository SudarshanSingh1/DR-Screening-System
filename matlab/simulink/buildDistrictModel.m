function buildDistrictModel()
    ...
    add_imf('Image Acquisition', 'sim_ImageAcquisition', '2', [x+100, y, x+100+w, y+h], '[0.85, 0.93, 1.0]');
    ...
    % 2. Network Delay
    add_imf('Bandwidth Upload Delay', 'sim_BandwidthDelay', '1', [x+100+w+60, y, x+100+w+60+w, y+h], '[0.9, 0.9, 0.9]');
    ...
    add_imf('Retinal Structure Segmentation', 'simulinkSegmentation', '32', [x+3*(w+gapX), y2, x+4*w+3*gapX, y2+h], '[0.85, 0.75, 0.95]');
    add_imf('Explainability Module', 'simulinkExplainability', '32', [x+4*(w+gapX), y2, x+5*w+4*gapX, y2+h], '[0.95, 0.85, 0.75]');
    add_imf('Referable DR Decision', 'simulinkReferableDecision', '32', [x+5*(w+gapX), y2, x+6*w+5*gapX, y2+h], '[1.0, 0.85, 0.80]');
    add_imf('Confidence Assessment', 'simulinkConfidenceCheck', '32', [x+6*(w+gapX), y2, x+7*w+6*gapX, y2+h], '[0.95, 0.85, 1.0]');
    add_imf('Clinical Decision Output', 'simulinkClinicalOutput', '32', [x+7*(w+gapX), y2, x+8*w+7*gapX, y2+h], '[0.75, 1.0, 0.75]');
    ...
    % CONNECTIONS
    % ========================================
    add_line(modelName, 'Clock/1', 'Image Acquisition/1', 'autorouting', 'smart');
    
    % Arrival -> Demux_Arr
    add_line(modelName, 'Image Acquisition/1', 'Demux_Arr/1', 'autorouting', 'smart');
    add_line(modelName, 'Demux_Arr/1', 'Bandwidth Upload Delay/1', 'autorouting', 'smart');
    ...
    add_line(modelName, 'Bandwidth Upload Delay/1', 'Mux_PQ/1', 'autorouting', 'smart');
    add_line(modelName, 'Bandwidth Upload Delay/1', 'Log_NetOut/1', 'autorouting', 'smart');
    ...
    add_line(modelName, 'AI Screening Engine/1', 'Retinal Structure Segmentation/1', 'autorouting', 'smart');
    add_line(modelName, 'Retinal Structure Segmentation/1', 'Explainability Module/1', 'autorouting', 'smart');
    add_line(modelName, 'Explainability Module/1', 'Referable DR Decision/1', 'autorouting', 'smart');
